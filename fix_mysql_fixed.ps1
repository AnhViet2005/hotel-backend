...
# Kiểm tra cổng 3306
$myPid = netstat -ano | Select-String ':3306' | Where-Object { $_ -match 'LISTENING' } | ForEach-Object { ($_ -split '\s+')[4] } | Select-Object -First 1
if ($myPid) {
    Write-Host "\nCổng 3306 đang được tiến trình PID $myPid sử dụng. Đang dừng tiến trình..."
